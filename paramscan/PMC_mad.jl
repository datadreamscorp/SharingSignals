...
#OUTFILE = "../data/mad_fit1.csv"
#OUTFILE = "../data/mad_fit2.csv"
OUTFILE = "../data/mad_fit3.csv"
#OUTFILE = "../data/mad_fit4.csv"
#OUTFILE = "../data/mad_fit5.csv"
...
@everywhere const R_REPLICATES = 1
...
function abc_smc(obs_df, cols;
    ...
    for t in 2:nrounds
        ...
        # Silverman multivariate rule-of-thumb - an alternative scaling rule
            h2 = (4.0/(D+2))^(2.0/(D+4)) * N^(-2.0/(D+4))
            sqrt(h2) .* σs
        elseif bandwidth_rule == :gelman
        # Gelman optimal MCMC scaling - using because theres no real rule for ABC
            scale = 2.38 / sqrt(D)
            scale .* σs
        else
